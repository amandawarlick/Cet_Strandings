...
test <- cetaceans_mapping_sp %>% group_by(Cetacean.Common.Name) %>%
  summarize(cnt = n_distinct(National.Database.Number))

cetaceans_mapping_sp_small <- cetaceans_mapping_sp %>%
  merge(test, by = 'Cetacean.Common.Name') %>%
  filter(cnt > 10 & Cetacean.Common.Name != 'Other')
...
geom_polygon(aes(x = long, y = lat, group = group), fill = "grey93", color = "grey50", size = 0.2) + 
  ...
  geom_point(data = cetaceans_mapping_sp,
...
geom_polygon(aes(x = long, y = lat, group = group), fill = "grey93", color = "grey50", size = 0.2) + 
  ...
  geom_point(data = cetaceans_mapping_sp_small_in %>%
  ...
  geom_density2d(data = cetaceans_mapping_sp_small_in  %>% filter(Water.Body == 'Inland_WA')
                 %>% filter(Cetacean.Common.Name != 'Sperm whale'), 
                 aes(x = long_corr, y = lat_corr), col = 'black', bins = 3, size = .5) +  
  #scale_alpha(range = c(0, 0.1), guide = FALSE) +
  xlab("") + ylab("") +
  facet_wrap(~ Cetacean.Common.Name, strip.position = "right") +
...
gganimate(strands_species_seas, interval = 2.0, filename = "~/Documents/R/Cet_Strandings/Figures/strands_species_an_seas.gif") 
```
...
gganimate(all_strands_year_an, interval = 1, filename = "~/Documents/R/Cet_Strandings/Figures/all_strands_year_an.gif")
