...
#2000-2018
covs_loc <- read.csv('copernicus_dat.csv') 
...
covs_cet <- complete(imp_sst,2) %>%
  ...
  merge(covs_loc, by = c('year', 'month')) %>%
...
covs_cet <- covs_cet %>% dplyr::select(-c(chl, sst_salish)) %>%
...
covs_an_cet <- covs_cet %>%
  ...
  dplyr::summarize(MEI = mean(MEI, na.rm = T), PDO = mean(PDO, na.rm = T),
                   ...
                   sst_sali = mean(sst_sali, na.rm = T), mld = mean(mld, na.rm = T),
...
```{r decompplot, eval=TRUE, echo=FALSE, fig=TRUE, fig.cap=''}
...
#indep obs unique var; best fit w/o covariates
...
#indep obs same var, not converging
fit.cet.3b <- MARSS::MARSS(cet_dat, model=list(
  ...
  R = "diagonal and equal",
  x0 = "unequal",
  tinitx=0), control = list(maxit = 5500))

#cor obs error with same variance and cor, not converging
...
MARSSparamCIs(fit.cet.3a)

# this doesn't look very correct or compelling
par(mfrow=c(2,2))
for(i in 1:4){
plot(ts(exp(fit.cet.3a$states[i,]), start = c(2003), end = c(2017), freq = 1),
        ylab="Strandings", xlab="", type="l")
lines(ts(exp(fit.cet.3a$states[i,])-1.96*exp(fit.cet.3a$states.se[i,]), start = c(2003),
         end = c(2017), freq = 1),
         type="l",lwd=1,lty=2,col="red")
lines(ts(exp(fit.cet.3a$states[i,])+1.96*exp(fit.cet.3a$states.se[i,]), start = c(2003),
         end = c(2017), freq = 1),
      type="l",lwd=1,lty=2,col="red")
points(ts(exp(fit.cet.3a$ytT[i,]), start = c(2003), end = c(2017), freq = 1),
...
fit.1 <- MARSS::MARSS(cet_dat[,1:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[2, , drop = F] #PDO
fit.2 <- MARSS::MARSS(cet_dat[,1:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[3, , drop = F] #sst
fit.3 <- MARSS::MARSS(cet_dat[,1:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[4, , drop = F] #upwell
fit.4 <- MARSS::MARSS(cet_dat[,1:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[5, , drop = F] #npgo
fit.5 <- MARSS::MARSS(cet_dat[,1:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[6, , drop = F] #chla
fit.6 <- MARSS::MARSS(cet_dat[,1:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[7, , drop = F] #salish sst
fit.7 <- MARSS::MARSS(cet_dat[,1:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[8, , drop = F] #mld
fit.8 <- MARSS::MARSS(cet_dat[,1:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[9, , drop = F] #uwnd
fit.9 <- MARSS::MARSS(cet_dat[,1:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[10, , drop = F] #vwnd
fit.10 <- MARSS::MARSS(cet_dat[,1:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[11, , drop = F] #ctd temp sst2
fit.11 <- MARSS::MARSS(cet_dat[,1:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[c(1,2,3,4,5,6,8,10), , drop = F] #full
fit.full <- MARSS::MARSS(cet_dat[,1:19], model=model.list, control = list(maxit = 1000))

######### lagged variables

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[1, 1:18, drop = F] #lagged MEI
fit.lagMEI <- MARSS::MARSS(cet_dat[,2:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[2, 1:18, drop = F] #lagged PDO
fit.lagPDO <- MARSS::MARSS(cet_dat[,2:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[3, 1:18, drop = F] #lagged sst
fit.lagsst <- MARSS::MARSS(cet_dat[,2:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[4, 1:18, drop = F] #lagged upwelling
fit.lagUP <- MARSS::MARSS(cet_dat[,2:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[5, 1:18, drop = F] #lagged NPGO
fit.lagNPGO <- MARSS::MARSS(cet_dat[,2:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[6, 1:18, drop = F] #lagged chla
fit.lagchl <- MARSS::MARSS(cet_dat[,2:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[7, 1:18, drop = F] #lagged salish
fit.lagsalish <- MARSS::MARSS(cet_dat[,2:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[8, 1:18, drop = F] #lagged mld
fit.lagmld <- MARSS::MARSS(cet_dat[,2:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[9, 1:18, drop = F] #lagged uwnd
fit.laguwnd <- MARSS::MARSS(cet_dat[,2:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[10, 1:18, drop = F] #lagged vwnd
fit.lagvwnd <- MARSS::MARSS(cet_dat[,2:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[11, 1:18, drop = F] #ctd temp sst2
fit.lagsst2 <- MARSS::MARSS(cet_dat[,2:19], model=model.list, control = list(maxit = 1000))

model.list <- list(B=B,U=U,Q=Q,Z=Z,A=A,R=R,D=D,d=d,C=C,x0=x0,tinitx=tinitx)
model.list$c <- cet_covs[c(6,8), 1:18, drop = F] #full
fit.lagfull <- MARSS::MARSS(cet_dat[,2:19], model=model.list, control = list(maxit = 1000))
...
#                                fit.lagMEI$AICc, fit.lagUP$AICc))
# aic_tab$delAIC <- aic_tab$AICc - min(aic_tab$AICc)

aic_tab_covs_cet <- data.frame(Covs = c('None', 'MEI', 'PDO', 'SST', 'Upwell', 'NPGO', 
                                        'chl', 'salish', 'mld', 'uwnd', 'vwnd', 'ctd_temp', 'full',
                                        'mei_lag', 'pdo_lag', 'sst_lag', 'upwell_lag', 'npgo_lag',
                                        'chl_lag', 'salish_lag', 'mld_lag', 'uwnd_lag', 'vwnd_lag', 
                                        'ctdtemp_lag', 'full lag'),
            AICc = c(fit.cet.3a$AICc, fit.1$AICc, fit.2$AICc, fit.3$AICc, 
                     fit.4$AICc, fit.5$AICc, fit.6$AICc, fit.7$AICc,
                     fit.8$AICc, fit.9$AICc, fit.10$AICc, fit.11$AICc, fit.full$AICc,
                     fit.lagMEI$AICc, fit.lagPDO$AICc, fit.lagsst$AICc,
                     fit.lagUP$AICc, fit.lagNPGO$AICc, fit.lagchl$AICc,
                     fit.lagsalish$AICc, fit.lagmld$AICc, fit.laguwnd$AICc,
                     fit.lagvwnd$AICc, fit.lagsst2$AICc, fit.lagfull$AICc))
...
MARSSparamCIs(fit.lagmld) #most relevant for gray whales and harbor porpoises
#MARSSparamCIs(fit.lagsst) #none
MARSSparamCIs(fit.lagUP) #gray whales/harbor (-), striped dolphins (+)
MARSSparamCIs(fit.lagchl) #gray whale, dalls -- negative, very small, all overlap 0
MARSSparamCIs(fit.lagsalish) #neg on harbor porps
#MARSSparamCIs(fit.lagvwnd) #none
#MARSSparamCIs(fit.lagsst2)

#lagged upwelling; looks much more reasonable than figure w/o covariates
par(mfrow=c(2,2))
for(i in 1:4){
plot(ts(exp(mod_best$states[i,]), start = c(2000), end = c(2017), freq = 1),
        ylab="Strandings", xlab="", type="l")
lines(ts(exp(mod_best$states[i,])-1.96*exp(mod_best$states.se[i,]), start = c(2000), 
         end = c(2017), freq = 1), type="l",lwd=1,lty=2,col="red")
lines(ts(exp(mod_best$states[i,])+1.96*exp(mod_best$states.se[i,]), start = c(2000), 
         end = c(2017), freq = 1), type="l",lwd=1,lty=2,col="red")
points(ts(exp(mod_best$ytT[i,]), start = c(2000), end = c(2017), freq = 1),
       lwd=0.1, pch = 19, col="grey")
title(rownames(cet_dat)[i])
}

plot(residuals(mod_best)$state.residuals[1,],
     ylab="State Residuals", xlab="")
abline(h=0)
title('Unique trends, equal variance, lagged upwelling')
...
# fit.cet.3aa <- MARSS::MARSS(cet_dat, model=mod.list, control=list(maxit=3500)) 
# 
# mod.list$R = "diagonal and equal"
# mod.list$Q = "diagonal and unequal"
# fit.cet.3ab <- MARSS::MARSS(cet_dat, model=mod.list, control=list(maxit=2000))
# 
# mod.list$R = "diagonal and equal"
# mod.list$Q = "equalvarcov"
# fit.cet.3ac <- MARSS::MARSS(cet_dat, model=mod.list, control=list(maxit=4000)) #not converging
# 
# mod.list$R = "diagonal and unequal"
# mod.list$Q = "diagonal and equal"
# fit.cet.3ba <- MARSS::MARSS(cet_dat, model=mod.list, control=list(maxit=3500)) #best fit, but residuals??
# 
# mod.list$R = "diagonal and unequal"
# mod.list$Q = "diagonal and unequal"
# fit.cet.3bb <- MARSS::MARSS(cet_dat, model=mod.list, control=list(maxit=4000)) #not converging
# 
# mod.list$R = "diagonal and unequal"
# mod.list$Q = "equalvarcov"
# fit.cet.3bc <- MARSS::MARSS(cet_dat, model=mod.list, control=list(maxit=3000))
# 
...
# par(mfrow=c(4,1))
# for(i in 1:4){
# plot.ts(ts(exp(fit.cet.3ba$states[i,]), start = c(2000,1), end = c(2018,12), freq = 12) ,ylab="Monthly Strandings", xlab="Time", type="l")
# lines(ts(exp(fit.cet.3ba$states[i,])-1.96*exp(fit.cet.3ba$states.se[i,]),
#          start = c(2000,1), end = c(2018,12), freq = 12), type="l",lwd=1,lty=2,col="red")
# lines(ts(exp(fit.cet.3ba$states[i,])+1.96*exp(fit.cet.3ba$states.se[i,]),
#          start = c(2000,1), end = c(2018,12), freq = 12), type="l",lwd=1,lty=2,col="red")
# points(ts(exp(fit.cet.3ba$ytT[i,]), start = c(2000,1), end = c(2018,12), freq = 12), lwd=0.1, pch = 19, col="grey")
...
# cet.fit.cov0 <- MARSS::MARSS(cet_dat, model=model.list, control=list(maxit=3000)) #gray whale and striped dolphin not converged
# 
