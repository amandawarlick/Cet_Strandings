...
covs_cet <- read.csv('CetCovs00_19.csv', header = T, stringsAsFactors = F) 
...
covs_an <- covs_cet %>%
  ...
  dplyr::summarize(MEI = mean(MEI, na.rm = T), PDO = mean(PDO, na.rm = T),
                   ...
                   NPGO = mean(NPGO, na.rm = T), chla = mean(chla, na.rm = T),
...
covs_all <- read.csv('covs_cet_lagged.csv', header = T, stringsAsFactors = F) %>%
...
hp1 <- mydraw.gam(mod_best)[[1]] + ggtitle('') +
  ...
  scale_y_continuous(limits = c(-2.3, 1.5), breaks = seq(-2, 1, 1))
...
hp2 <- mydraw.gam(mod_best)[[4]] + ggtitle('') + 
  ...
  annotate('text', label = paste('edf','=','4.4,', 'p', '<', '0.01'), 
                        x = 2011, y = -2.3, size = 3, parse = F) +
  scale_y_continuous(limits = c(-2.3, 1.5), breaks = seq(-2, 1, 1))
  
hp3 <- mydraw.gam(mod_best)[[2]] + ggtitle('') + 
  xlab('PDO') + ylab('') + 
  plot_theme(plot.title = element_text(size = 12),
             axis.title.y = element_blank()) + 
...
hp4 <- mydraw.gam(mod_best)[[3]] + ggtitle('') + 
...
mod_full <- gam(cnt ~ s(month, bs = 'cc') + s(year) +
              # s(PDO) +
              # s(PDO_lag3) +
              # s(PDO_lag) +
              # s(MEI) +
...
gw1 <- mydraw.gam(mod_best)[[1]] + ggtitle('') +
  xlab('Month') + ylab('Effect') +
    annotate('text', label = paste('edf','=','5.3,', 'p', '<', '0.01'), 
             x = 8, y = -2.5, size = 3.3, parse = F) +
  plot_theme(plot.title = element_text(size = 12)) + 
  scale_x_continuous(breaks = c(1,3,5,7,9,11), labels = c(1,3,5,7,9,11)) +
  scale_y_continuous(limits = c(-2.5, 2.5), breaks = c(-3:2))
...
gw2 <- mydraw.gam(mod_best)[[2]] + ggtitle('') + 
  ...
  plot_theme(plot.title = element_text(size = 12),
             axis.title.x = element_blank(),
             axis.title.y = element_blank()) + 
  annotate('text', label = paste('edf','=','5.7,', 'p', '<', '0.01'), 
                        x = 2013, y = -2, size = 3.3, parse = F) +
...
gw3 <- mydraw.gam(mod_best)[[4]] + ggtitle('') + 
  ...
  plot_theme(plot.title = element_text(size = 12),
             axis.title.y = element_blank()) +
  annotate('text', label = paste('edf','=','0.8,', 'p', '<', '0.05'), 
                        x = 0.6, y = -2, size = 3.3, parse = F) +
...
plot_grid(gw1, gw2, gw4, gw3, nrow = 1)
...
str1 <- mydraw.gam(mod_best)[[1]] + ggtitle('') +
  xlab('Month') + ylab('Effect') +
    annotate('text', label = paste('edf','=','3.4,', 'p', '<', '0.01'), 
...
str2 <- mydraw.gam(mod_best)[[4]] + ggtitle('') + 
  xlab('') + ylab('') + 
  plot_theme(plot.title = element_text(size = 12),
             axis.title.y = element_blank(), 
             axis.title.x = element_blank()) + 
...
str3 <- mydraw.gam(mod_best)[[2]] + ggtitle('') +
  ...
  scale_y_continuous(limits = c(-5.5, 2), breaks = c(-5:2)) +
...
plot_grid(str1, str2, str4, str3, nrow = 1)
...
plot_grid(da1, da2, da3, da4, nrow = 1)
...
AIC <- data.frame(AIC(mod0, mod1, mod2)) %>%
  transform(delAIC = AIC-min(AIC(mod0, mod1, mod2))) %>%
  transform(delAIC = round(delAIC, 2), AIC = round(AIC, 2))
...
hump3 <- mydraw.gam(mod_best)[[3]] + ggtitle('') + 
  ...
  plot_theme(plot.title = element_text(size = 12),
             axis.title.y = element_blank()) + 
annotate('text', label = paste('edf','=','1.3,', 'p', '=', '0.12'), 
...
hump4 <- mydraw.gam(mod_best)[[4]] + ggtitle('') + 
  ...
  plot_theme(plot.title = element_text(size = 12),
             axis.title.y = element_blank()) + 
  annotate('text', label = paste('edf','=','2.0,', 'p', '<', '0.05'), 
                        x = 1, y = -2.8, size = 3, parse = F) +
  scale_y_continuous(limits = c(-2.8, 2))
  
plot_grid(hump1, hump2, hump4, hump3, nrow = 1)
