...
cetaceans <- read.csv("cetacean_data_CleaningOuput00_18_updated.csv", header = TRUE, na.strings = "", stringsAsFactors = FALSE) %>% 
  #filter(Cetacean.Common.Name != 'Harbor porpoise') %>% 
  #filter(Water.Body == 'Inland_WA')
  transform(Season = ifelse(Month.of.Observation == 'DEC' | Month.of.Observation == 'JAN' | Month.of.Observation == 'FEB', 'Winter', 
                            ifelse(Month.of.Observation == 'MAR' | Month.of.Observation == 'APR' | Month.of.Observation == 'MAY', 'Spring', 
                                   ifelse(Month.of.Observation == 'JUN' | Month.of.Observation == 'JUL' | Month.of.Observation == 'AUG', 'Summer', 
                                          ifelse(Month.of.Observation == 'SEP' | Month.of.Observation == 'OCT' | Month.of.Observation == 'NOV', 'Fall', 'NA'))))) %>%
  transform(sci_name = paste(Genus, Species, sep = " ")) %>%
  transform(State = toupper(State)) %>%
  transform(Sex = ifelse(Sex == 'Unknown', 'Unid', Sex))
...
age_sex_table <- age_sex_all %>% 
  ...
  arrange(Age.Class) %>%
  dplyr::select(c(Age.Class, cnt, Female, Prop.Fem, Male, Prop.Male, Unid, Prop.Unid)) 
...
data_by_year <- cetaceans %>% #filter(Cetacean.Common.Name == 'Minke whale') %>%
  group_by(Year.of.Observation, Cetacean.Common.Name) %>%
  dplyr::summarize(cnt = n_distinct(National.Database.Number)) %>%
  filter(Cetacean.Common.Name %in% c('Humpback whale', 'Gray whale', 'Harbor porpoise', 'Dalls porpoise',
                                     'Pacific white-sided dolphin', 'Striped dolphin'))
...
ggplot(data_by_year, aes(x = Year.of.Observation, y = cnt)) + 
  ...
  plot_theme(strip.background = element_blank(),
             panel.border = element_rect(fill = NA)) +
  facet_wrap(~Cetacean.Common.Name, scale = 'free_y') +
...
ids <- cetaceans %>% dplyr::select(National.Database.Number, sci_name)

numbers <- sum(duplicated(cetaceans$National.Database.Number))

#paper table
species_tot <- cetaceans %>%
  group_by(sci_name) %>%
  summarize(tot = n_distinct(National.Database.Number)) %>%
  arrange(desc(tot))

species_state <- cetaceans %>%
  filter(State != 'NA') %>%
  group_by(sci_name, State) %>%
  summarize(cnt = n_distinct(National.Database.Number)) %>%
  dcast(sci_name ~ State, value.var = 'cnt') %>%
  transform(OR = ifelse(is.na(OR), 0, OR),
            WA = ifelse(is.na(WA), 0, WA)) %>%
  merge(species_tot, by = 'sci_name')

species_sex <- cetaceans %>%
  group_by(sci_name, Sex) %>%
  summarize(cnt = n_distinct(National.Database.Number)) %>%
  dcast(sci_name ~ Sex, value.var = 'cnt') %>%
  #dplyr::select(-N) %>%
  merge(species_state, by = 'sci_name')

species_HI <- cetaceans %>%
  filter(Findings.of.Human.Interaction == 'Y') %>%
  group_by(sci_name) %>%
  summarize(HI = n_distinct(National.Database.Number)) 

species_tab <- species_sex %>% merge(species_HI, by = 'sci_name', all = T) %>%
  dplyr::select(sci_name, tot, OR, WA, Male, Female, Unid, HI)

write.csv(species_tab, 'species_tab.csv', row.names = F)

seasonal_sp <- cetaceans %>%
  #filter(Cetacean.Common.Name != 'Unidentified') %>%
  group_by(Season, sci_name) %>%
  summarize(cnt = n_distinct(National.Database.Number)) %>%
  merge(species_tot, by = 'sci_name') 

seasonal_sp_wide <- seasonal_sp %>%
  melt(id.vars = c('sci_name', 'Season', 'tot')) %>%
  dcast(sci_name + tot ~ Season, value.var = 'value')

seasonal_sp_wide[is.na(seasonal_sp_wide)] <- 0

seasonal_sp_wide <- seasonal_sp_wide %>%
  transform(fall_p = round(Fall/tot, 2), spring_p = round(Spring/tot, 2),
            summer_p = round(Summer/tot, 2), winter_p = round(Winter/tot, 2)) %>%
  select(sci_name, tot, Spring, spring_p, Summer, summer_p, Fall, fall_p, Winter, winter_p)

setwd("~/Documents/R/Cet_Strandings/manuscript_figs")
write.csv(seasonal_sp_wide, "seasonal_sp.csv", row.names = F)
  
#paper figure
monthly_sp_dat <- cetaceans %>%
  filter(sci_name != 'NA NA') %>%
  filter(Cetacean.Common.Name %in% c('Humpback whale', 'Gray whale', 'Harbor porpoise', 'Dalls porpoise', 'Pacific white-sided dolphin', 'Striped dolphin')) %>%
  group_by(Month.of.Observation, Cetacean.Common.Name) %>%
  summarize(cnt = n_distinct(National.Database.Number)) 

ggplot(monthly_sp_dat, aes(x = as.numeric(Month.of.Observation), y = cnt)) +
  geom_line(linetype = 'dashed') + 
  geom_smooth(se = T) +
  geom_point(size = 0.8) +
...
#not summed
monthly_sp_dat2 <- cetaceans %>%
  filter(sci_name != 'NA NA') %>%
  filter(Cetacean.Common.Name %in% c('Humpback whale', 'Gray whale', 'Harbor porpoise', 'Dalls porpoise', 'Pacific white-sided dolphin', 'Striped dolphin')) %>%
  group_by(Month.of.Observation, Cetacean.Common.Name, Year.of.Observation) %>%
  summarize(cnt = n_distinct(National.Database.Number)) 

ggplot(monthly_sp_dat2, aes(x = as.numeric(Month.of.Observation), y = cnt)) +
  # geom_line(aes(linetype = 'dashed') +
  geom_smooth(se = T) +
  # geom_point(size = 0.8) +
  ylab("Monthly Strandings") + xlab("") + 
  facet_wrap(~Cetacean.Common.Name, scales = 'free_y') +
  plot_theme(strip.background = element_blank(),
             panel.border = element_rect(fill = NA)) + 
  scale_x_continuous(breaks = c(1,3,5,7,9,11), 
...
species <- cetaceans %>%
  group_by(sci_name) %>%
  summarize(cnt = n_distinct(National.Database.Number)) %>%
  transform(perc = round(cnt/nrow(cetaceans), 3)) %>%
  arrange(desc(cnt))
...
species_year %>% filter(Cetacean.Common.Name == 'Humpback whale')
...
species_figure <- species_year %>%
  #filter(sci_name != 'Unidentified') %>%
  ggplot(aes(Year.of.Observation, cnt, color = sci_name, group = sci_name)) +
  geom_line() + 
  #geom_smooth(se = F) +
...
summary(lm(cnt ~ Year.of.Observation + sci_name, data = species_year))

species_figure_box <- species_year %>%
  #filter(sci_name != 'Unidentified') %>%
  ggplot(aes(sci_name, cnt)) +
  geom_boxplot() +
  scale_y_continuous(labels = comma) +
  xlab("") + ylab("Annual Strandings") +
  plot_theme()
...
year_month_state <- cetaceans %>%
  filter(Cetacean.Common.Name %in% c('Harbor porpoise', 'Humpback whale', 'Gray whale')) %>%
  #filter(State != 'NA') %>%
  group_by(Year.of.Observation, Month.of.Observation, Cetacean.Common.Name) %>%
  summarize(cnt = n_distinct(National.Database.Number))

year_state_boxplot <- 
  ggplot(year_month_state, aes(x = as.factor(Year.of.Observation), y = cnt)) +
  geom_boxplot() + facet_grid(~Cetacean.Common.Name) +
...
species_prop_seas <- cetaceans %>%
  group_by(Cetacean.Common.Name, Season) %>%
  summarize(cnt = n_distinct(National.Database.Number))  %>%
  dcast(Cetacean.Common.Name ~ Season, value.var = 'cnt')
species_prop_seas[is.na(species_prop_seas)] <- 0
# species_prop_seas <- species_prop_seas %>%
