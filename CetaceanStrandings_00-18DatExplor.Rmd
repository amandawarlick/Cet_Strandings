...
cetaceans <- read.csv("cetacean_data_CleaningOuput00_18.csv", header = TRUE, na.strings = "", stringsAsFactors = FALSE) %>% 
  #filter(Cetacean.Common.Name != 'Harbor porpoise') %>% 
  #filter(Water.Body == 'Inland_WA')
  transform(Season = ifelse(Month.of.Observation == 'DEC' | Month.of.Observation == 'JAN' | Month.of.Observation == 'FEB', 'Winter', ifelse(Month.of.Observation == 'MAR' | Month.of.Observation == 'APR' | Month.of.Observation == 'MAY', 'Spring', ifelse(Month.of.Observation == 'JUN' | Month.of.Observation == 'JUL' | Month.of.Observation == 'AUG', 'Summer', ifelse(Month.of.Observation == 'SEP' | Month.of.Observation == 'OCT' | Month.of.Observation == 'NOV', 'Fall', 'NA'))))) %>%
...
monthly_species_figure <- 
  ...
  group_by(Month.of.Observation, sci_name) %>%
  summarize(cnt = n_distinct(National.Database.Number)) %>%
  ggplot(aes(x = factor(Month.of.Observation), y = cnt, 
          group = sci_name, color = sci_name)) +
  geom_line(aes(group = sci_name)) + 
  #geom_smooth(se = F) + 
  #geom_point() +
...
species_year <- cetaceans %>%
  filter(sci_name != 'NA NA') %>%
  #filter(Findings.of.Human.Interaction == 'Y') %>%
  group_by(Year.of.Observation, sci_name) %>%
  summarize(cnt = n_distinct(National.Database.Number)) 
